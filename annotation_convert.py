...
# pre-cleanup before running:  cd base_dir (see below); rm -rf labels_tmp classes.txt labels
...
collection = 'microfilm_training'  # examples:  test validation train 
...
# ToDo: if generated_classes_file already exists, then overwrite or rename or exit? 
...
#   we do not need to read this file since dir with b&w masks contains all info we need to
#    make color masks.


#
#    For each example jpg, process corresponding png b&w masks created by VGG to make a composite color mask.
...
for basename in os.listdir(masks_proto_dir):
    ...
    # save the color_mask
    sys.stdout.write(f"write {basename}.png\n")
