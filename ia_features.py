...
class Features:
    ...
    determine actions to take durning postprocessing.
    ...
    training vectors are save()'ed for input to an sklearn model in ia_post_model.py.
    ...
    def __init__(self, ground_page_type_path: str, training_set_out_path: str, max_rects_per_label: int = 4):
        ...
        if ground_page_type_path is not None:
            ...
            self._ground_df = pd.read_csv(ground_page_type_path, header=0, sep=',', quotechar='"',
                dtype={'file_basename':np.str, 'page_type':np.int})
            # load it into a dict
    ...
    def start(self, basename: str):
        ...
        # Big enough to hold _max_rects_per_class instances of _nclasses (labels).
    ...
    #   twice, first with ith=0 and then with ith=1. ith must not exceed max_rects_pre_class-1.
    def put(self, predicted_label: int, ith: int, region_size: float, confidence: float, aspect_ratio: float,
        ...
        :param predicted_label: 0=background, 1=title, 2=authors, 3=refs, 4=toc (0 is never used)
        :param ith: 0 thru max_rects_pre_class-1 indicating which instance of a label/class this is
    ...
    def _get_dummy_instance(self):
        return {"region_size": 0, "confidence": 0.0, "aspect_ratio": 0.0,
             "normalized_x": 0.0, "normalized_y": 0.0, "tag_rect_x0": 0,
             "tag_rect_y0": 0, "tag_rect_x1": 0, "tag_rect_y1": 0}
